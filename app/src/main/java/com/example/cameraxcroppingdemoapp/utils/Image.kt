package com.example.cameraxcroppingdemoapp.utils
...
import com.example.cameraxcroppingdemoapp.fragments.CameraFragment
import java.io.File
...
fun storeImage(image: Bitmap, photoFile: File): Uri {
    try {
        Log.d("LogForGirish", "photoFile :" + photoFile.path)
        val fos = FileOutputStream(photoFile)
        image.compress(Bitmap.CompressFormat.PNG, 90, fos)
        fos.close()
    } catch (e: FileNotFoundException) {
        Log.d(CameraFragment.TAG, "File not found: " + e.message)
        Log.d("LogForGirish", "File not found: " + e.message)
    } catch (e: IOException) {
...
fun cropImage(fullImage: Bitmap, previewSize: Size, cardFinder: Rect): Bitmap {
    ...
    Log.d("LogForGirish","Inside image.cropImage")
